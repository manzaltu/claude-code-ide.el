...
| ~claude-code-ide-buffer-font~                   | Custom font for Claude Code buffers         | ~nil~                                  |
...
(setq claude-code-ide-vterm-render-delay 0.032)  ; Increase for smoother but less responsive
#+end_src

This optimization detects rapid terminal redraw sequences (like when Claude expands text areas) and batches them for smoother rendering. The 16ms default delay provides optimal visual quality with imperceptible latency. Users with severe flickering can try 0.032 to 0.050.
...
You can set a custom font for Claude Code buffers, which may help if your default font doesn't render Claude's output well:

#+begin_src emacs-lisp
;; Use a specific font for Claude Code buffers
(setq claude-code-ide-buffer-font "Source Code Pro")

;; Or with a size specification
(setq claude-code-ide-buffer-font "Iosevka-12")

;; Disable (use default font) - this is the default
(setq claude-code-ide-buffer-font nil)
#+end_src
